    ...
    try:
        ...
        for protocol in protocols_data:
            ...
            if result.upserted_id:
                logging.info(
                    f"Inserted new protocol '{protocol.get('name')}': with unique ID '{protocol.get('protocol_id')}'"
                )
            else:
                logging.info(
                    f"Protocol already exists '{protocol.get('name')}': with unique ID '{protocol.get('protocol_id')}'"
                )
