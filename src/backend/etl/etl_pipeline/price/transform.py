...
def transform_ohlc_data(
    contract_metadata: dict, symbol: str, raw_data: pd.DataFrame
) -> list:
    """
    ...
    raw_data.fillna(
        {"Open": 0, "High": 0, "Low": 0, "Close": 0, "Volume": 0}, inplace=True
    )
    ...
    transformed_data = (
        raw_data[["Datetime", "Open", "High", "Low", "Close", "Volume"]]
        .apply(
            lambda row: {
