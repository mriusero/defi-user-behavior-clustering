...
def load_data_to_mongodb(data: list):
    ...
    """
    prices_collection = get_mongo_collection(db_name='defi_db', collection_name='market')

    prices_collection.create_index(
        [("timestamp", 1), ("symbol", 1), ("contract_address", 1)],
        unique=True
    )
    operations = []
    for record in data:
    ...
    if operations:
        ...
        logging.info(f"Inserted {result.upserted_count} new records into defi_db.market collection.")
    else:
