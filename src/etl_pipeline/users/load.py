...
def load_batch(batch_data: list, db_name: str, collection_name: str) -> None:
    ...
    """
    users_collection = get_mongo_collection(db_name=db_name, collection_name=collection_name)
...
def load_users_data(users_data: dict) -> None:
    ...
    logging.info(f"Preparing to load {len(users_data)} users in {len(data_chunks)} batches.")
    ...
    with Pool(processes=cpu_count()) as pool:
        ...
        for _ in tqdm(pool.imap(process_chunk, chunk_data_tuples), total=len(data_chunks), desc="Processing batches"):
            pass
