...
def upsert_transactions(protocol_name, contract_type, contract_id, blockchain, transactions):
    ...
    transactions_collection.create_index([("transaction_hash", 1)])  # Index by transaction hash
    transactions_collection.create_index([("timestamp", 1)])  # Index by timestamp

    bulk_operations = []

    for tx in transactions:
        value = float(tx["value"]) / 10 ** 18 if "value" in tx else None        # Convert from wei to ETH (1 ETH = 10^18 wei)
        gas = float(tx.get("gas", 0)) if "gas" in tx else None                  # Gas used in the transaction
        gas_used = float(tx.get("gasUsed", 0)) if "gasUsed" in tx else None     # Gas actually used (0 if not available)
        is_error = tx.get("isError", "0")                                       # If the transaction failed, 'isError' is "1", otherwise "0"
        ...
        transaction_doc = {
            "timestamp": datetime.utcfromtimestamp(int(tx["timeStamp"])),       # Convert Unix timestamp to datetime object
            "metadata": {
        ...
        bulk_operations.append(
            UpdateOne(
                {"transaction_hash": tx["hash"]},         # Use the transaction hash to identify the document
                {"$setOnInsert": transaction_doc},      # Insert if the transaction doesn't already exist
