...
def fetch_transactions_for_range(contract_address: str, start_block: int, end_block: int):
    ...
    while current_start <= end_block:
        ...
        logging.info(f"Fetching transactions for blocks {current_start} to {current_end}")
        try:

            txs = fetch_transactions(contract_address, current_start, current_end)      # API call for the current block range
            transactions.extend(txs)
            logging.info(f"Fetched {len(txs)} transactions for blocks {current_start}-{current_end}")
...
def fetch_all_transactions_parallel(contract_address: str, start_block: int, end_block: int, max_workers: int = 4):
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        while current_start <= end_block:
            current_end = min(current_start + block_step - 1, end_block)
            futures.append(executor.submit(fetch_transactions_for_range, contract_address, current_start, current_end))
...
def process_ethereum_contracts(start_date: str, end_date: str):
    ...
    logging.info(f"Fetching transactions between blocks {start_block} and {end_block}...")
    ...
    for contract in ethereum_contracts:
        ...
        logging.info(f"---- Processing contract '{contract_address}' for protocol '{protocol_name}':")

        try:
            transactions = fetch_all_transactions_parallel(contract_address, start_block, end_block)
            logging.info(f"Fetched {len(transactions)} total transactions for contract {contract_address}")

            upsert_transactions(protocol_name, type, contract_id, blockchain, transactions)
            logging.info(
            ...
            logging.error(f"Error fetching or processing transactions for contract {contract_address}: {e}")
            continue